...
const pontuacao = [28, 12, 66, 94, 22, 11, 74, 26, 44]
const string = [10, 20, 20, 8, 25, 3, 0, 30, 1]
const string2 = [10, 20, 20, 8, 25, 3, 2, 30, 1, 0]
function comparador(x) {
    let placar = [0, 0]
    let maiorPontuacao = x[0]
    let menorPontuacao = x[0]
    for (let p in x) {
        if (x[p] > maiorPontuacao) {
            placar[0] += 1
            maiorPontuacao = x[p]
        }
        if (x[p] < menorPontuacao) {
            placar[1] = parseInt(p) + 1
            menorPontuacao = x[p]
        }
    }
    return placar
